...
function RegionNode({
  ...
  return (
    <div className="w-full min-w-0 select-none">
      <div onClick={() => onSelect(isSelected ? null : node.id)} className={cn("flex items-start gap-2 px-3 py-2.5 rounded-xl transition-all duration-200 cursor-pointer group mb-1 w-full min-w-0 box-border", isSelected ? "bg-primary text-white shadow-lg shadow-primary/20" : isRoot ? "bg-slate-50/50 dark:bg-slate-800/50 text-slate-900 dark:text-slate-100 font-bold hover:bg-slate-100 dark:hover:bg-slate-800" : "hover:bg-slate-50 dark:hover:bg-slate-800/50 text-slate-600 dark:text-slate-400")} style={{ paddingLeft: `${depth * 16 + 12}px` }}>
        ...
        {getIcon()}
        <span className={cn("text-sm break-all whitespace-normal min-w-0 flex-1 tracking-tight leading-snug", isRoot ? "text-base font-black uppercase" : "font-semibold", isSelected ? "text-white" : "text-slate-700 dark:text-slate-300")}>{node.name}</span>
      </div>
