...
interface ClippedMonitoringMapProps {
  devices: Device[];
  className?: string;
}

// Quezon Province focus configuration
const QUEZON_CENTER: [number, number] = [14.25, 121.9];
const QUEZON_BOUNDS: L.LatLngBoundsExpression = [
  [13.1, 121.2], // Southwest
  [15.3, 123.0], // Northeast
];

// World bounding box for the mask
const WORLD_OUTER: [number, number][] = [
  [90, -180], [90, 180], [-90, 180], [-90, -180]
];

/**
 * High-Precision Quezon Province Mainland Boundary
 * Used to "punch a hole" in the world mask to reveal the map only within this shape.
 */
const QUEZON_HOLE: [number, number][] = [
...
const ClippedMonitoringMap = ({ devices, className }: ClippedMonitoringMapProps) => {
  ...
  // Dashboard background colors to match for perfect masking
  ...
  return (
    <div className={cn("relative w-full h-full rounded-3xl overflow-hidden border border-slate-200 dark:border-white/5", className)}>
      ...
      >
        ...
        {/* ABSOLUTE MASK: Covers everything but Quezon */}
        <Polygon
          positions={[WORLD_OUTER, QUEZON_HOLE]}
          pathOptions={{
            fillColor: maskColor,
            fillOpacity: 1.0,
            stroke: false,
        ...
        {/* Cyber-style border for Quezon */}
        <Polygon
          positions={QUEZON_HOLE}
          pathOptions={{
            color: '#22c55e',
            weight: 2,
            fillOpacity: 0,
...
export default memo(ClippedMonitoringMap);
